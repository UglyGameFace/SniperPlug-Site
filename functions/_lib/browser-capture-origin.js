...
export function requireWhopAppFrameCaptures(input) {
  ...
  for (const capture of captures) {
    if (!isWhopAppFrameUrl(capture?.pageUrl || capture?.frameUrl)) {
      throw new HttpError(422, 'Browser capture was rejected because it did not come from a rendered HTTPS Whop app frame. Reopen the individual content page and capture it again.');
    }
  }
