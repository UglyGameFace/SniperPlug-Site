...
export async function saveGuideDraft(env, principalValue, id, input) {
  ...
  const snapshot = await snapshotGuide(env, principalId, id);
  if (!snapshot) throw new HttpError(404, 'Guide draft not found in this account workspace.');
  ...
  try {
    return await saveGuideDraftWithCleanup(env, principalId, id, input);
  } catch (error) {
    const db = await ensureImporterWorkspaceSchema(env);
    const current = await db.prepare('SELECT * FROM guides WHERE principal_id = ? AND id = ?').bind(principalId, id).first().catch(() => null);
    ...
    try {
      await restoreGuideSnapshot(env, principalId, snapshot, { expectedUpdatedAt: current.updated_at });
