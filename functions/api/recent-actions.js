...
export async function onRequest(context) {
  try {
    ...
    if (context.request.method === 'GET') {
      return json(ownerFacingHistory(await listRecentActions(context.env, account)));
    }
    ...
    if (body.action === 'dismiss') return json(ownerFacingResult(await dismissRecentActions(context.env, account, body)));
    return json(ownerFacingResult(await undoRecentActions(context.env, account, body)));
  } catch (error) {
