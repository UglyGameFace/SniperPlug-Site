...
export async function onRequest(context) {
  try {
    ...
    if (context.request.method === 'GET') {
      return json({ job: await latestBulkJob(context.env, admin) });
    }
    ...
    if (action === 'start') {
      return json({ job: await startBulkJob(context.env, admin, body) }, 201);
    }
    if (action === 'step') {
      ...
      return json({ job: await stepBulkJob(context.env, admin, whop, id) });
    }
    if (action === 'cancel') {
      const id = String(body?.jobId || '').trim();
      if (!id) throw new HttpError(422, 'Choose a bulk job to cancel.');
      return json({ job: await cancelBulkJob(context.env, admin, id) });
    }
