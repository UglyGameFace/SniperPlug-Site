...
assert.ok(runtime.includes('POST_PAGE_SIZE = MOBILE_QUERY.matches ? 4 : 10') && runtime.includes("dataset.action = 'post-load-more'") && runtime.includes('state.postRenderLimit'), 'Large content scans do not stay behind explicit bounded pagination.');
...
assert.ok(runtime.includes('GUIDE_PAGE_SIZE = MOBILE_QUERY.matches ? 8 : 24') && runtime.includes('filteredGuideIds') && runtime.includes('guide-load-more'), 'The review queue still renders every guide at once.');
...
assert.ok(runtime.includes('activeOperationKeys') && runtime.includes('requestAnimationFrame(() => requestAnimationFrame(resolve))'), 'Async actions can rerun before visible feedback paints.');
