...
const decisionLock = read('assets/js/control-center-decision-lock.js');
...
assert.ok(lifecycle.includes('control-center-decision-lock.js'), 'The per-record decision lock is not loaded for cached Control Center sessions.');
assert.ok(decisionLock.includes("root.addEventListener('click'") && decisionLock.includes('stopImmediatePropagation'), 'Overlapping opposite decisions are not blocked before the main runtime handles them.');
assert.ok(decisionLock.includes("source:${card.dataset.experienceId}") && decisionLock.includes("post:${card.dataset.sourceKey}"), 'Decision locks are not scoped to the exact source or content item.');
assert.ok(decisionLock.includes('125_000') && decisionLock.includes("pagehide"), 'Decision locks can remain stuck after timeout or navigation.');
assert.ok(!decisionLock.includes('MutationObserver'), 'Decision hardening reintroduced broad DOM observation churn.');

for (const file of [
  'functions/_lib/source-policy.js',
  'functions/_lib/posts.js',
  'functions/api/control.js',
  'assets/js/control-center-decision-lock.js',
...
console.log('✓ Opposite decisions for one source or item cannot overlap in the browser.');
